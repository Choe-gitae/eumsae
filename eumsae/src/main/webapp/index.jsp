<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8" />
        <title>Insert title here</title>
    </head>
    <body>
        <a href="front/managemant-base">관리자페이지 베이스 페이지</a><br />
        <a href="front/management-main">관리자페이지 메인 페이지</a><br />
        <a href="front/managemant-manager">관리자페이지 관리자 계정 관리 페이지</a><br />
        <a href="front/managemant-user">관리자페이지 회원 관리 페이지</a><br />
        <a href="front/managemant-product">관리자페이지 상품 관리 페이지</a><br />
        <a href="front/managemant-sales">관리자페이지 매출 관리 페이지</a><br />
        <a href="management/product-managemant">관리자페이지 상품 관리 페이지</a><br />
        <a href="front/managemant-setting">관리자페이지 설정 페이지</a><br />
        <hr />
        <hr />
        <hr />
        <hr />
        <hr />
        <a href="main.jsp">메인페이지</a><br />
        <a href="shop/lp-list">홈페이지 상품 리스트 페이지</a><br />
    </body>
</html>
